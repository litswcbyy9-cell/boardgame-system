...
let RecommendationsService = class RecommendationsService {
    constructor(prisma, tablesService) {
        this.prisma = prisma;
    ...
    async recommendTables(partySize, startAt, endAt) {
        return this.tablesService.matchTables(partySize, startAt, endAt);
    }
    buildGameReason(row, query) {
        const parts = [];
        if (Number(row.peopleScore) >= 90)
            parts.push(`适合 ${query.partySize} 人`);
        else
            parts.push('人数略有偏差但仍可安排');
        if (Number(row.durationScore) >= 80)
            parts.push(`时长接近 ${query.minutes} 分钟`);
        if (query.category && row.category === query.category)
            parts.push(`匹配${row.category}偏好`);
        if (Number(row.historyScore) >= 90)
            parts.push('会员历史记录高度匹配');
        else if (Number(row.historyScore) >= 70)
            parts.push('会员曾偏好同类游戏');
        if (Number(row.hotScore) >= 50)
            parts.push('近期热度较高');
        if (!parts.length)
            parts.push('综合人数、时长和门店权重后排序靠前');
        return parts.join('，') + '。';
    }
};
